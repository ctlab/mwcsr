...
SolverCardinality::SolverCardinality(Instance& _instance, int _maxIterations):SolverLag(_instance, _maxIterations),
weightLast{0.0},weightOutside{0.0}
{
...
bool SolverCardinality::primalHeuristic()
{
	...
	for(unsigned myIter=0;myIter<iter;++myIter)
	{
		...
		vector<int> hop=vector<int> (instance.nNodes,std::numeric_limits<int>::max());
		...
		while(!myPQueue.empty() && currentSize<params.cardCons)
		{
			...
			if(!myHeurTerminalsBool[k] || inComponentBool[k])
			{
				...
				{
					if (distance[l] > distance[k]+toAdd+epsOpt || (distance[l] == distance[k]+toAdd && hop[l]>hop[k]+1) )
			...
			else
			{
				...
				while(!inComponentBool[currentNode])
				{
					...
					add++;
					if(currentSize+add>params.cardCons)
					{
						fit=false;
						break;
					}
		...
		if(best>incumbentObj && false)
		{
			...
			myPQueue=priority_queue<nodevaluepair,std::vector<nodevaluepair>,std::greater<nodevaluepair> >();

			pred=vector<int>(instance.nNodes,-1);
			extracted=vector<bool>(instance.nNodes,false);
			distance=vector<double> (instance.nNodes,std::numeric_limits<double>::max());
			...
			int countTerm=0;
			currentSize=1;
			while(!myPQueue.empty() && numBest>0 && currentSize<params.cardCons)
			{
				nodevaluepair k2=myPQueue.top();
				myPQueue.pop();
				int k=k2.id;
		...
		do
		{
			...
			{
				if(currentSize>=params.cardCons)
					break;
...
int SolverCardinality::lagrangianPegging()
{
	...
	for(int i=0;i<instance.nNodes;++i)
	{
		if(fixedToZero[i] || fixedToOne[i] || fabs(realPrizes[i]-epsOpt)<epsOpt)
			continue;
...
double SolverCardinality::calculateCurrentSolution(bool save)
{
	...
	for(int i=0;i<instance.nNodes;++i)
	{
		...
		if(fixedToOne[i])
		{
			...
			if(save && currentSolution[i])
				sumSolution[i]++;
			if(solutionSize>params.cardCons)
			{
				cout<<"strange"<<endl;
			}
			continue;
	...
	for(;i<myNV.size();++i)
	{
		...
		solutionSize++;
		//cout<<nv.id<<" "<<realPrizes[nv.id]<<" "<<solutionSize<<" "<<instance.myPrizes[nv.id]<<endl;
