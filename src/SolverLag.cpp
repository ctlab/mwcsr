...
bool cutToRemove(SolverLag::cut x)
{
	return x.toRemove;
}

SolverLag::SolverLag(Instance& _instance, int _maxIterations):instance(_instance),
myCuts {list<cut>()},
myNewCuts {list<cut>()},
realPrizes {vector<double>(instance.nNodes,0)}, currentSolution {vector<double>(instance.nNodes,0)},
previousSolution {vector<double>(instance.nNodes,0)},
sumSolution {vector<int>(instance.nNodes,0)},
incumbent {vector<bool>(instance.nNodes,0)},dualIncumbent {vector<int>(instance.nNodes,0)},
labels {vector<int>(instance.nNodes,0)},
fixedToZero {vector<int>(instance.nNodes,0)},fixedToOne {vector<int>(instance.nNodes,0)},
incumbentObj {0.0},
subgradientSquared {1},
subgradientNorm {0},
directionPrevSquared {0},
alpha {params.beta}, noImprov {0}, numberOfComponents {0},
...
worstBoundCFT  {std::numeric_limits<double>::lowest()},
 counterCFT {0},
 maxIterations {_maxIterations},
 iterations {0},
 sepIter(params.sepIter),
 sepIterFreeze(params.sepIterFreeze),
 inRins(false),
 savedObj(0.0),
 runtime(0.0)
{
...
int SolverLag::solveSubgradient(int maxIterations)
{
	...
	iterations=0;
	//if(params.outputlag)
	//	cout<<"fixed "<<costBasedFixing()<<" variables to zero due to cost in component"<<endl;
	...
	while(iterations<maxIterations && sqrt(subgradientSquared)>epsOpt)
	{
		...
		//cout<<currentBound<<" "<<previousBound<<" "<<(currentBound>previousBound)<<endl;

		//double boundPCSTP=0.0;

		if(currentBound<bestBound)
		{
			//cout<<currentBound<<" "<<bestBound<<" "<<(currentBound>bestBound)<<endl;
		...
		if((boundImprov) && params.pegging>0 && !inRins)
		{
			...
			{
				if(instance.maxRevenueInComponent[c]<incumbentObj && !instance.componentFixed[c])
				{
		...
		//cout<<bestBound<<" "<<currentBound<<" "<<incumbentObj<<" "<<bestBoundCheck<<endl;
		...
		if(params.outputlag)
		{
			...
			cout<<std::setprecision(9)<<"iteration: \t"<<iterations<<"\t lagrangian bound: \t"<<
									instance.transformInternalValue(bestBound)<<"\t current bound: \t "<<
									instance.transformInternalValue(currentBound)<<"\t incumbent: \t "<<
									instance.transformInternalValue(incumbentObj)<<"\t number of active cuts: \t"<<numberOfCuts<<endl;
		}
		if(nFixed)
			myCuts.erase(std::remove_if(myCuts.begin(), myCuts.end(), cutToRemove),	myCuts.end());
		upgradeMultipliers();
		for(int i=0;i<instance.nNodes;++i)
		{
	...
	if(params.outputlag)
	{
		cout<<"finished"<<endl;
		cout<<std::setprecision(9)<<"iteration: \t"<<iterations<<"\t lagrangian bound: \t"<<
										instance.transformInternalValue(bestBound)<<"\t incumbent: \t "<<
...
void SolverLag::writeStatistics()
{
	...
	instance.gapLag=(instance.transformInternalValue(bestBound)-instance.transformInternalValue(incumbentObj))/instance.transformInternalValue(incumbentObj)*100;
	if(params.inputformat==0)
	{
		instance.gapLag=(instance.transformInternalValue(incumbentObj)-instance.transformInternalValue(bestBound))/instance.transformInternalValue(bestBound)*100;
	}
...
void SolverLag::updateMultipliersSherali()
{
	if(noImprov>params.betaIter)
	{
		...
		alpha/=2;
		if(params.outputlag)
			cout<<"no improvement for"<< params.betaIter<<" iterations, new alpha is "<<alpha<<endl;
	...
	//cout<<theta<<" "<<alpha<<" "<<incumbentObj<<" "<<directionSquared<<" "<<subgradientSquared<<endl;

	for(cut& c: myCuts)
	{
		if(c.frozen)
			continue;
		//cerr<<c.lambda<<" "<<c.lambda-theta*c.direction<<" "<<c.direction<<endl;
...
void SolverLag::updateMultipliersLucena()
{
	...
	//cout<<subgradientSquared<<endl;
	//cout<<theta<<" "<<incumbentObj<<" "<<alpha<<" "<<currentBound<<" "<<subgradientSquared<<endl;
	//incumbentObj=7;
	...
	for(cut& c: myCuts)
	{
		if(c.frozen)
				continue;
		//cerr<<"cut "<<c.lambda<<" "<<c.lambda-theta*c.subgradient<<" "<<c.subgradient<<endl;
...
void SolverLag::updateMultipliersCFT()
{
	...
	else
	{
		...
		if(cftBounds.size()>100)
		{
			...
			if(oldBest-bestBound<0.01*delta)
			{
				...
				//cout<<avgBound<<" "<<bestBound<<" "<<delta<<" "<<0.001*delta<<endl;
	...
	for(cut& c: myCuts)
	{
		if(c.frozen)
			continue;
		//cerr<<c.lambda<<" "<<c.lambda-theta*c.subgradient<<" "<<c.subgradient<<endl;
		c.lambda=std::max(0.0,c.lambda-theta*c.subgradient);
	}
...
int SolverLag::checkPreviousCuts(bool addCuts)
{
	...
	for(cut& c: myCuts)
	{
		...
		if(nFixedToZero==c.lhs.size() && c.rhs.size()==1 && addCuts)
		{
			...
			for(nodevaluepair n: c.rhs)
			{
				...
				//if(incumbent[n.id])
				//	cout<<n.id<<" "<<incumbent[n.id]<<" "<<c.lhs.size()<<" "<<instance.adjList[n.id].size()<<endl;
			}
			c.toRemove=true;
			c.subgradient=0;
		...
		else
		{
			...
			if(c.lambda==0 && c.subgradient==0 && c.directionPrevious>0 && c.age>params.maxAge)
			{
				//directionPrevSquared-=(c.directionPrevious*c.directionPrevious);
		...
		//cout<<"prev "<<c.subgradient<<" "<<c.violated<<" "<<lhs<<" "<<subgradientSquared<<" "<<c.age<<endl;
	}

	//numberOfCuts+=myNewCuts.size();
	if(addCuts)
	{
		//myNewCuts.erase(std::remove_if(myNewCuts.begin(), myNewCuts.end(), cutToRemove),	myNewCuts.end());
	...
	//myCuts.remove_if([&](const cut& c){ return (c.lambda==0.0 && c.subgradient==0 ); } );
	//cerr<<myCuts.size()<<endl;
...
int SolverLag::setVariableFixing(const vector<int>& toZero,const vector<int>& toOne)
{
...
int SolverLag::separateCuts()
{
	...
	for(int n=0;n<instance.nNodes;++n)
	{
		...
		if(currentSolution[n]==1 && instance.realTerminals[n] && labels[n]==0)
		{

			CompStruct myComponentHelper;
			vector<bool> componentBoundary=vector<bool>(instance.nNodes,false);
			...
			while(!myQueue.empty())
			{
				...
				for(int li:instance.adjList[k])
				{
					...
					else if(labels[li]!=myCurrentLabel)
					{
						...
						{
							//cout<<li<<" "<<myComponentHelper.sumPrize<<" "<<instance.myPrizes[li]<<endl;
			...
			//cout<<"myComponent: "<<myComponent.size()<<" myCurrentLabel: "<<myCurrentLabel<<endl;
	...
	std::sort(myComponents.begin(),myComponents.end(),std::greater<CompStruct>());
	...
	for(unsigned i=0;i<myComponents.size()-1;++i)
	{
		...
		if(other>=myComponents.size())
			other=0;
		//cout<<myComponents[i].sumPrize<<" "<<myComponents[other].sumPrize<<endl;
		...
		for(unsigned j=0;j<myComponents[i].components.size();++j)
		{
			...
			if(!instance.realTerminals[myComponents[i].components[j]])
				continue;
			rhsPrize=realPrizes[myComponents[i].components[j]]/(max(instance.myBudgetCost[myComponents[i].components[j]],0.00001));
		...
		if(myComponents[i].sumPrize+epsInt >=incumbentObj|| params.separation==0)
		{
			nodevaluepair m;
			m.value=1.0;
			...
			for(unsigned j=0;j<myComponents[other].components.size();++j)
			{
				...
				if(!instance.realTerminals[myComponents[other].components[j]])
					continue;
				//cout<<"j "<<j<<" "<<components[other][j]<<endl;
				//rhsPrize=instance.myPrizes[myComponents[other].components[j]];
				rhsPrize=realPrizes[myComponents[other].components[j]]/(max(instance.myBudgetCost[myComponents[other].components[j]],0.00001));
		...
		if(params.separation==0)
		{
			...
			int otherBoundarySize=myComponents[other].boundaryIndexed.size();
			...
			while(!myQueue.empty() && otherBoundaryFound<otherBoundarySize)
			{
				int m=myQueue.front();
				//int m=myQueue.top();
		...
		{
			for(unsigned j=0;j<myComponents[i].boundaryIndexed.size();++j)
			{
		...
		if(myCut.rhs.size()==2)
		{
			...
			n.id=myCut.rhs[0].id;
			dummy.push_back(n);
			long helper1=myCut.myHasher(myCut.lhs,dummy,instance.nNodes);
			...
			dummy2.push_back(n2);
			long helper2=myCut.myHasher(myCut.lhs,dummy2,instance.nNodes);
		...
		//cout<<myCut.rhs.size()<<" "<<myCut.lhs.size()<<" "<<myCut.rhsConst<<endl;
