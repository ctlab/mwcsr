...
template<>
struct std::hash<relax::Variable> {
...
template<>
struct std::hash<relax::VariableSum> {
    std::size_t operator()(relax::VariableSum const& s) {
        size_t seed = 0;
        std::hash<relax::Variable> hasher;
        std::vector<size_t> hashes;
        std::transform(s.summands.cbegin(), s.summands.cend(), std::back_inserter(hashes), [&hasher](relax::Variable const& v){
            return hasher(v);
        });
