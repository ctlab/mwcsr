...
:SolverLag(_instance, _maxIterations),
 M{vector<vector<double>>(instance.nNodes)}
{
...
double SolverBudget::calculateCurrentSolution(bool saveSol)
{
	...
	long myCobj=combo(&myItems[0],&myItems[instance.nNodes], C, 0, 99999999, 1, 0);
	...
	for(int j = 0; j <trueNodes; j++){
		for(int i = 1; i <= C; i++){
			if(j>0)
			{
				...
				{
					double value=myEntries[trueNodes*(i-instance.myBudgetCost[jf[j]])+j-1]+realPrizes[jf[j]];
	...
	{
		if ((j2==0 && myEntries[j2+weight2*trueNodes]>0) || (j2>0 && myEntries[j2+weight2*trueNodes] != myEntries[j2-1+weight2*trueNodes]))
		{
			//cout<<j<<endl;
			...
			//test+=realPrizes[jf[j2]];
			//cout<<j<<" "<<jf[j]<<" "<<realPrizes[jf[j]]<<" "<<instance.myBudgetCost[jf[j]]<<" "<<weight<<endl;
			weight2 = weight2-instance.myBudgetCost[jf[j2]];
		}
		--j2;
	}

	//cout<<myBound<<" "<<myEntries[myEntries.size()-1]<<" "<<test<<" "<<C<<" "<<weight2<<endl;
...
/*
	...
	for(int i = 1; i <= C; i++){
		...
		for(int j = 0; j <trueNodes; j++){
			if(j > 0){
				M[j][i] = M[j-1][i];
				if (instance.myBudgetCost[jf[j]] <= i)
					M[j][i] = max(M[j][i], M[j-1][i-instance.myBudgetCost[jf[j]]]+realPrizes[jf[j]]);
			}
			else{
				M[j][i] = 0;
				if(instance.myBudgetCost[jf[j]] <= i)
					M[j][i] = max(M[j][i], realPrizes[jf[j]]);
			}
		}
	}
	...
	while(j>=0 && weight>=0)
	{
		if ((j==0 && M[j][weight]>0) || (j>0 && M[j][weight] != M[j-1][weight]))
		{
			//cout<<j<<endl;
...
//	cout<<budgetIn<<endl;
	//cout<<objHeur<<" "<<objKnapsack<<" "<<objSpanning<<" "<<currentBound<<endl;
	for(int n=0;n<instance.nNodes;++n)
	{
		if(inItems[n]>0)
			continue;
		...
		{
			//cout<<last<<" "<<myIndex<<" "<<order.size()<<" "<<additionalCost<<" "<<instance.myBudgetCost[last]<<" "<<inItems[last]<<" "<<addedObj<<" "<<realPrizes[last]<<endl;

			if(additionalCost-instance.myBudgetCost[last]*inItems[last]>=0)
			{
...
//			cout<<inItems[last]<<endl;
		...
		{
			cout<<"bug "<<addedObj<<" "<<objHeur<<" "<<additionalCost<<endl;
			exit(1);
		}
		...
		{
			cout<<"fixing "<<realPrizes[n]<<" "<<myBound+addedObj<<" "<<incumbentObj<<" "<<myBound+objHeur<<endl;
			fixedToZero[n]=true;
			instance.nFixedZero++;
			nFixed++;
		}
...
bool SolverBudget::primalHeuristic()
{
	...
	for(unsigned myIter=0;myIter<iter;++myIter)
	{
		...
		priority_queue<nodevaluepair, vector<nodevaluepair>, greater<nodevaluepair>> myPQueue;
		...
		while(!myPQueue.empty())
		{
			...
			int k=nv.id;
			//cout<<k<<" "<<distance[k]<<budget<<" "<<extracted[k]<<" "<<pred[k]<<endl;
			...
			if(!myHeurTerminalsBool[k] || inComponentBool[k])
			{
				...
				for(int l: instance.adjList[k])
				{
					...
					if(budget+instance.myBudgetCost[l]>instance.budget)
						continue;
					//cout<<distance[l]<<" "<< distance[k]<<" "<<instance.myBudgetCost[l]<<" "<<lpSolution[l]<<" "<<l<<" "<<k<<endl;
					if (distance[l] > distance[k] + instance.myBudgetCost[l]*(1-lpSolution[l])+epsOpt)
					//if (distance[l] > distance[k] + toAdd*(1-lpSolution[l])+epsOpt)
					{
						//cerr<<distance[l]<<" "<<distance[k]+ instance.myBudgetCost[l]*(1-lpSolution[l])+epsOpt<<" "<<l<<" "<<k<<endl;

						distance[l] = distance[k] + instance.myBudgetCost[l]*(1-lpSolution[l])+epsOpt;
						//distance[l] = distance[k] + toAdd*(1-lpSolution[l])+epsOpt;
						pred[l] = k;
			...
			else
			{
				...
				{
					//cout<<currentNode<<" "<<instance.myBudgetCost[currentNode]<<" "<<pred[currentNode]<<endl;
		...
		do
		{
			...
			for(int n=0;n<instance.nNodes;++n)
			{
				...
				{
					if(!inComponentBool[j] && instance.myPrizes[j]>0 && budget+instance.myBudgetCost[j]<=instance.budget)
					{
						nodevaluepair nv;
						nv.id=j;
						//nv.value=instance.myPrizes[j];
						nv.value=instance.myPrizes[j]/instance.myBudgetCost[j];
						toAdd.push_back(nv);
					}
				}
			}
...
bool SolverBudget::primalHeuristic()
{
			...
			else if(instance.trueTerminals[k])
			{
				...
				/*else
				{
					...
					while(!inComponent[currentNode])
					{
						...
						//cout<<currentNode<<" "<<extracted[currentNode]<<endl;
	...
	priority_queue<nodevaluepair, vector<nodevaluepair>, less<nodevaluepair>> myPQueue2;
	while(!myQueue.empty())
	{
		int k=myQueue.front();
		myQueue.pop();
