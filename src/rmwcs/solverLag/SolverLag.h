...
public:
	...
	struct cut{
		...
		std::size_t myHasher(std::vector<nodevaluepair> const& vec,std::vector<nodevaluepair> const& vec2, int offset) const {
		  std::size_t seed = 0;
		  for(auto& i : vec) {
		    seed ^= ((std::size_t)i.id) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
		  }
		  for(auto& i : vec2) {
			seed ^= ((std::size_t)i.id+offset) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
		  }
		  return seed;
		}
	};
...
public:
	...
	inline double getBestBound() const
	{
		return bestBound;
	}

	inline double getIncumbentObj() const
	{
		return incumbentObj;
	}

	inline vector<bool> getIncumbent() const
	{

		return incumbent;
	}

	inline list<SolverLag::cut> getCuts() const
	{
		return myCuts;
	}
